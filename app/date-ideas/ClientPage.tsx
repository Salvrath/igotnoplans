...
import { IDEAS, type Idea, type Budget, type Mood, type TimeWindow } from "@/lib/ideas";
...
export default function ClientPage() {
  ...
  const candidates = useMemo(() => {
    return IDEAS
      ...
      .slice(0, 30)
      .map((x) => x.idea);
  }, [budget, city, indoorsOk, mood, outdoorsOk, timeWindow]);

  const [current, setCurrent] = useState<Idea | null>(null);

  function generate() {
    const pool = candidates.length ? candidates : IDEAS.filter((i) => i.useCase === "date");
    setCurrent(pickOne(pool));
  }

  async function share() {
    const url = typeof window !== "undefined" ? window.location.href : "https://igotnoplans.com";
    const text = "Need date ideas? Try this:";
    try {
      if (navigator.share) {
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-50">
      <div className="mx-auto max-w-3xl px-4 py-10">
        <header className="mb-8">
          ...
          <h1 className="mt-2 text-4xl font-semibold tracking-tight">Date ideas, instantly.</h1>
          <p className="mt-3 text-zinc-300">No overthinking. Just pick a vibe and go.</p>
        </header>
        ...
        <section className="mt-6">
          {current ? (
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-6">
              ...
              <div className="mt-6 text-sm text-zinc-400">Tip: share the link to decide together.</div>
            </div>
          ) : (
